...
public class AbstractEntityLinker {
    ...
    private Set<String> classFileNames = new HashSet<String>();
    ...
    public AbstractEntityLinker(String dbo_Class,Set<String> classFileNames, Integer windowSize, Integer nGram, Map<String, TreeMap<String, List<String>>> alphabetInfo) throws Exception {
        ...
        if (this.classFileNames.isEmpty()) {
            throw new Exception("no entities of the class is found for annotation!!");
        } else {
            for (String dbo_ClassName : classFileNames) {
                String classDir = FileFolderUtils.getClassDir(dbo_ClassName) + "/";
                String rawFilesDir = dbpediaDir + classDir + "rawFiles/";
                String inputFile = dbpediaDir + classDir;
                String outputDir = dbpediaDir + classDir + patternDir;
                List<File> rawFiles = FileFolderUtilsAnnotation.getFiles(rawFilesDir, ".json");
                if (!rawFiles.isEmpty()) {
                    Map<String, List<DBpediaEntity>> fileDBpediaEntities = FileFolderUtilsAnnotation.readAlphabetSplitTables(rawFiles);
                    addPatterns(dbo_Class,outputDir, fileDBpediaEntities, windowSize, nGram);
                } else {
                    throw new Exception("There is no files in " + inputFile + " to generate properties!!");
                }
            }
        }
    }
